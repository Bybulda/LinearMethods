...
from coursework.constants_equations.Constants import G1, R
...
def equation1(x, y, lst_g):
    ...
    res = 101325/(R*1100) * (2 * np.exp(arg1) + np.exp(arg2) + np.exp(arg3) + 2 * np.exp(arg4)) - 58.79828073827122
    return res
...
def equation2(x, y, lst_g):
    ...
    arg3 = x + y - lst_g[2] / (R*1100)
    arg4 = 2 * x + y - lst_g[3] / (R*1100)
    res = 101325/(R*1100) * (2 * np.exp(arg1) + np.exp(arg2) + np.exp(arg3) + np.exp(arg4)) - 58.79828073827122
    return res
...
plt.contour(X1, X2, F1, levels=1, cmap='viridis')
plt.contour(X1, X2, F2, levels=1, cmap='viridis')
